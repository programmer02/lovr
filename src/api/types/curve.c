...
static int l_lovrCurveGetPoint(lua_State* L) {
  ...
  lovrAssert(index >= 0 && index < (int) lovrCurveGetPointCount(curve), "Invalid Curve point index: %d", index + 1);
  float point[3];
...
static int l_lovrCurveAddPoint(lua_State* L) {
  ...
  usize index = lua_isnoneornil(L, i) ? lovrCurveGetPointCount(curve) : luaL_checkinteger(L, i) - 1;
