...
static void renderNode(Model* model, u32 nodeIndex, u32 instances) {
  ModelNode* node = &model->data->nodes[nodeIndex];
  mat4 globalTransform = model->globalNodeTransforms + 16 * nodeIndex;

  if (node->primitiveCount > 0) {
    f32 pose[16 * MAX_BONES];
...
Model* lovrModelInit(Model* model, ModelData* data) {
  ...
  model->globalNodeTransforms = malloc(16 * sizeof(f32) * model->data->nodeCount);
...
static void applyAABB(Model* model, u32 nodeIndex, f32 aabb[6]) {
  ModelNode* node = &model->data->nodes[nodeIndex];
  ...
  for (u32 i = 0; i < node->primitiveCount; i++) {
    ...
    if (position && position->hasMin && position->hasMax) {
      ...
      f32 xa[3] = { position->min[0] * m[0], position->min[0] * m[1], position->min[0] * m[2] };
      f32 xb[3] = { position->max[0] * m[0], position->max[0] * m[1], position->max[0] * m[2] };

      f32 ya[3] = { position->min[1] * m[4], position->min[1] * m[5], position->min[1] * m[6] };
      f32 yb[3] = { position->max[1] * m[4], position->max[1] * m[5], position->max[1] * m[6] };

      f32 za[3] = { position->min[2] * m[8], position->min[2] * m[9], position->min[2] * m[10] };
      f32 zb[3] = { position->max[2] * m[8], position->max[2] * m[9], position->max[2] * m[10] };
...
void lovrModelGetAABB(Model* model, f32 aabb[6]) {
  ...
  updateGlobalNodeTransform(model, model->data->rootNode, (f32[]) MAT4_IDENTITY);
